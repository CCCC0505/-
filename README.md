...
这是一个围绕“建档 -> 问卷诊断 -> 画像 -> 推荐 -> 练习 -> 新画像”构建的学习平台原型。当前版本聚焦初二数学场景，用更稳定的规则建模和更透明的智能分析，帮助老师与学生快速看清当前状态、训练方向与变化轨迹。
...
- 当前仅支持初二数学样例库
- 题量固定为 84 题：诊断题 24、练习题 60
- 主链路是“建档 -> 问卷诊断 -> 画像 -> 推荐 -> 练习 -> 新画像”

当前版本范围边界：

- 当前仅开放初二数学样例题库与画像训练闭环
- 教师端、家长端、多学科、多年级为后续扩展
...
- `/api/system/question-bank/stats`
- `/api/cold-start/sessions`
- `/api/cold-start/{session_id}/questionnaire`
- `/api/cold-start/{session_id}/diagnostic/submit`
- `/api/cold-start/{session_id}/finalize`
- `/api/practice/recommendations`
- `/api/practice/answers`
- `/api/students/{student_id}/wrong-questions`
- `/api/ui/subjects`
- `/api/ui/knowledge-points`

关键用例包括：

- 做错一道推荐题后，`wrong-questions` 返回 1 条真实记录
- `subjects` 接口返回“数学可用，其他学科后续开放”
